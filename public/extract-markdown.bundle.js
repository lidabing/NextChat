/*
  Bundled script: Readability + Turndown + small runner
  Purpose: single-file script you can paste into browser console on sites
  with strict CSP (no external script loading). After pasting, call
  `extractMarkdownToFile({ filename: 'my-article.md' })` to download the
  extracted article as Markdown.

  Generated from local `Readability.js` and `turndown.min.js` in the repo.
*/

/* ==== Readability.js (inlined) ==== */
...
/* Readability contents copied from public/Readability.js */
...
/* ==== Wrapper / runner ==== */
(function (global) {
  function extractMarkdownToFile(options) {
    options = options || {};
    var filename = options.filename || (document.title ? document.title.replace(/[\\/:*?"<>|]/g, '') + '.md' : 'article.md');

    // Readability and Turndown are inlined, so they should exist
    if (typeof Readability === 'undefined' || typeof TurndownService === 'undefined') {
      throw new Error('Bundled Readability or TurndownService not available.');
    }

    var docClone = document.implementation.createHTMLDocument(document.title || '');
    var base = docClone.createElement('base');
    base.href = document.baseURI;
    docClone.head.appendChild(base);
    try { docClone.head.innerHTML += document.head.innerHTML; } catch (e) { }
    try { docClone.body.innerHTML = document.body.innerHTML; } catch (e) { }

    var reader = new Readability(docClone);
    var article = reader.parse();
    if (!article) throw new Error('Readability failed to extract main content.');
  ...
  global.extractMarkdownToFile = extractMarkdownToFile;

  if (typeof console !== 'undefined') {
    console.log('extract-markdown.bundle.js loaded — call extractMarkdownToFile({filename:"article.md"})');
  }
})(window);
